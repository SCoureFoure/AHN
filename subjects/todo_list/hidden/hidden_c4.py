"""Hidden judge suite — cumulative through cycle 4 (full suite). Locked with todo_list subject."""
import pytest
...
def test_hidden_c3_remove_pending_item():
    t = TodoList()
    t.add("a"); t.add("b"); t.add("c")
    t.remove("b")
    assert t.list_items() == ["a", "c"]
    assert t.pending() == ["a", "c"]


def test_hidden_c3_remove_done_item():
    t = TodoList()
    t.add("a"); t.add("b")
    t.complete("a")
    t.remove("a")
...
def test_hidden_c3_remove_not_found_raises():
    t = TodoList()
    t.add("a")
    with pytest.raises(ValueError):
        t.remove("z")
...
def test_hidden_c3_remove_preserves_order():
    t = TodoList()
    t.add("a"); t.add("b"); t.add("c")
    t.remove("b")
    assert t.list_items() == ["a", "c"]
...
def test_hidden_c4_higher_priority_first():
    t = TodoList()
    t.add("lo", priority=0)
    t.add("hi", priority=2)
    assert t.list_items() == ["hi", "lo"]


def test_hidden_c4_equal_priority_insertion_order():
    t = TodoList()
    t.add("a", priority=1)
    t.add("b", priority=1)
    assert t.list_items() == ["a", "b"]
...
def test_hidden_c4_default_priority_backward_compat():
    t = TodoList()
    t.add("x")
    assert t.list_items() == ["x"]


def test_hidden_c4_pending_sorted_by_priority():
    t = TodoList()
    t.add("lo", priority=0)
    t.add("hi", priority=1)
    assert t.pending() == ["hi", "lo"]


def test_hidden_c4_list_items_includes_done_in_priority_order():
    t = TodoList()
    t.add("hi", priority=2)
    t.add("lo", priority=0)
    t.complete("lo")
